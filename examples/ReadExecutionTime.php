...
$request->Set(GatewayRequest::EXPIRE_YEAR(), "2027");
...
//
// Prepare curlResponseCallback for execution times
//
$curlResponseCallback = function($handle, $body, $request, $response, $headers) {
   if ($request->Get(GatewayRequest::TRANSACTION_TYPE()) != "CC_CONFIRM") {
      $response->Set("X-CURLINFO_CONNECT_TIME", curl_getinfo($handle, CURLINFO_CONNECT_TIME));
      $response->Set("X-CURLINFO_TOTAL_TIME", curl_getinfo($handle, CURLINFO_TOTAL_TIME));
      $response->Set("X-CURLINFO_HTTP_CODE", curl_getinfo($handle, CURLINFO_HTTP_CODE));
   }
};
$service->SetCurlResponseCallback($curlResponseCallback);
...
if ($service->PerformPurchase($request, $response)) {
  ...
  print "CURLINFO_CONNECT_TIME: " . $response->Get("X-CURLINFO_CONNECT_TIME") . "\n";
  print "CURLINFO_TOTAL_TIME: " . $response->Get("X-CURLINFO_TOTAL_TIME") . "\n";
  print "CURLINFO_HTTP_CODE: " . $response->Get("X-CURLINFO_HTTP_CODE") . "\n";
  print "------------------------------------------\n";
} else {
